...
impl VerifierParams {
    ...
    pub fn split(
        self,
    ) -> Result<(VerifierParamsSplitCommon, VerifierParamsSplitSpecific), ZkpError> {
        Ok((
...
pub fn load_srs_params(size: usize) -> Result<KZGCommitmentSchemeBN254, ZkpError> {
    let srs = SRS.ok_or(ZkpError::MissingSRSError)?;
    ...
    if size > 16384 {
        return Err(ZkpError::ParameterError);
    }
