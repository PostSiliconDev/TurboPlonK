...
impl<F: PrimeField> PlonkChallenges<F> {
    ...
    pub(super) fn insert_beta_gamma(&mut self, beta: F, gamma: F) -> Result<(), ZkpError> {
        if self.challenges.is_empty() {
            self.challenges.push(beta);
            self.challenges.push(gamma);
            Ok(())
        } else {
            Err(ZkpError::ChallengeError)
        }
    }

    /// Insert alpha.
    pub(super) fn insert_alpha(&mut self, alpha: F) -> Result<(), ZkpError> {
    ...
    /// Insert zeta.
    pub(super) fn insert_zeta(&mut self, zeta: F) -> Result<(), ZkpError> {
    ...
    /// Insert u.
    pub(super) fn insert_u(&mut self, u: F) -> Result<(), ZkpError> {
    ...
    pub(super) fn get_beta_gamma(&self) -> Result<(&F, &F), ZkpError> {
    ...
    /// Return alpha.
    pub(super) fn get_alpha(&self) -> Result<&F, ZkpError> {
        if self.challenges.len() > 2 {
    ...
    /// Return zeta.
    pub(super) fn get_zeta(&self) -> Result<&F, ZkpError> {
        if self.challenges.len() > 3 {
    ...
    /// Return u.
    pub(super) fn get_u(&self) -> Result<&F, ZkpError> {
        if self.challenges.len() > 4 {
...
pub(super) fn t_poly<PCS: PolyComScheme, CS: ConstraintSystem<PCS::Field>>(
    ...
    let k_inv = k[1].inverse().ok_or(ZkpError::DivisionByZero)?;
...
pub(crate) fn split_t_and_commit<R: CryptoRng + RngCore, PCS: PolyComScheme>(
    ...
    n: usize,
) -> Result<(Vec<PCS::Commitment>, Vec<FpPolynomial<PCS::Field>>), ZkpError> {
    ...
    for i in 0..n_wires_per_gate {
        ...
        let (cm_t, t_poly) = if let Some(lagrange_pcs) = lagrange_pcs {
            ...
            let q_eval = FpPolynomial::fft(&sub_q, max_power_of_2).ok_or(ZkpError::FFTError)?;
            let q_eval = FpPolynomial::from_coefs(q_eval);

            let cm = lagrange_pcs
                .commit(&q_eval)
                .map_err(|_| ZkpError::CommitmentError)?;
