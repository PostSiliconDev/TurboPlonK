...
pub fn indexer<PCS: PolyComScheme, CS: ConstraintSystem<PCS::Field>>(
    cs: &CS,
    pcs: &PCS,
) -> Result<PlonkProverParams<PCS>, ZkpError> {
...
pub fn indexer_with_lagrange<PCS: PolyComScheme, CS: ConstraintSystem<PCS::Field>>(
    ...
    let domain =
        FpPolynomial::<PCS::Field>::evaluation_domain(n).ok_or(ZkpError::GroupNotFound(n))?;
    let domain_m = FpPolynomial::<PCS::Field>::quotient_evaluation_domain(m)
        .ok_or(ZkpError::GroupNotFound(m))?;
...
impl PlonkProof<KZGCommitmentSchemeBN254> {
    ...
    pub fn from_bytes_be<CS: ConstraintSystem<Fr>>(bytes: &[u8]) -> Result<Self, ZkpError> {
        ...
        if bytes.len() < bytes_len {
            return Err(ZkpError::DeserializationError);
        }
