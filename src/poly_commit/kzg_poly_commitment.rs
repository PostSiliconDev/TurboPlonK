...
impl<P: Pairing> KZGCommitmentScheme<P> {
    ...
    pub fn to_unchecked_bytes(&self) -> Result<Vec<u8>, ZkpError> {
        let mut bytes = vec![];
    ...
    pub fn from_unchecked_bytes(bytes: &[u8]) -> Result<Self, ZkpError> {
        if bytes.len() < 8 {
        ...
        for i in 0..len_1 {
            ...
            let g1 = P::G1::deserialize_with_mode(reader, Compress::No, Validate::No)
                .map_err(|_| ZkpError::DeserializationError)?;
            p1.push(g1);
        }
...
impl<'b> PolyComScheme for KZGCommitmentSchemeBN254 {
    ...
    fn commit(&self, polynomial: &FpPolynomial<Fr>) -> Result<Self::Commitment, ZkpError> {
        let coefs = polynomial.get_coefs_ref();

        let degree = polynomial.degree();

        if degree + 1 > self.public_parameter_group_1.len() {
            return Err(ZkpError::DegreeError);
        }
    ...
    fn verify(
        ...
        if left_pairing_eval == right_pairing_eval {
            ...
            Err(ZkpError::PCSProveEvalError)
        }
    }
